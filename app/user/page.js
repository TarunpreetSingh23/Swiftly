...
const handleCancel = async (orderId) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.order_id === orderId ? { ...t, is_canceled: true } : t
        )
      );
    } else {
            ...
            {tasks.map((task) => (
              ...
              >
                <div className="flex justify-between items-start flex-wrap gap-4">
                  <div>
                    ...
                    <p className="text-sm">
                      ...
                      <span className={`font-semibold ${
                        ...
                        task.is_approved ? "text-blue-600" :
                  ...
                  {!task.is_completed && !task.is_canceled && !is_approved (
                    <button
...
>
  Cancel
                ...
                <div className="mt-2 text-sm text-gray-700">
                  <p><span className="font-medium">Scheduled:</span> {task.date} | {task.timeSlot}</p>
                  <p><span className="font-medium">Phone:</span> {task.phone}</p>
                  <p><span className="font-medium">Address:</span> {task.address}</p>
                </div>
              </li>
            ))}
          </ul>
